...
* **NWP** (`delta_store.nwp`) stores physical-unit `Float32` with plain ZSTD-3 and parquet's
  ...
  data takes about 1 minute to download and convert. The writer-property comparison behind the
  format choice is in
  [PR #271](https://github.com/openclimatefix/nged-substation-forecast/pull/271); the ~58 GB per
  year is the measured average of the 899 daily runs on disk today, which is higher than the
  figure that PR extrapolated.
