...
off the spread. This is the **incumbent** — the method our models will be compared against — so we
reproduce it faithfully as the `nged_incumbent` [baseline
...
judgement. If a single deterministic number is needed, NGED probably pick the 95th percentile of all
the values. That 95th percentile is a deliberately **conservative** operating point: the tool's job
is to warn when demand might approach the network's flex/firm capacity limit, so erring high is the
safe direction.
