...
six products in this table and four more from the survey below.** Every other product on this page
is surveyed only, except where the "Scored in" column says otherwise. "Roadmap status" copies the
product's row in the [Weather data](../roadmap/data-sources.md#weather-data) catalogue; a version
label such as v0.5 names a [roadmap milestone](../roadmap/index.md#milestones).
