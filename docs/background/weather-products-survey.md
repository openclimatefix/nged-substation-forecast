...
**This page is a survey, not a plan: it records which weather products could join three studies,
and commits the project to ingesting none of them.** The survey asked which further forecast models,
reanalyses, and satellite retrievals could join the two studies of past weather, and which archives
could support [issue 810](https://github.com/openclimatefix/nged-substation-forecast/issues/810),
an open study of which weather model gives the best power forecast at the lead times the live
service runs at. The list of products the project ingests, plans to ingest, or is researching as a
possible source is the catalogue under [Weather data](../roadmap/data-sources.md#weather-data).

**The live forecast depends on one weather product, ECMWF's ensemble forecast (ENS) as republished
by Dynamical.org**, a non-profit that republishes weather-model archives as cloud-hosted Zarr and
Icechunk stores. ERA5 (🚧 v0.5) and CAMS (🚧 v0.7) are planned for training and offline capacity
estimation, not for the serving path. The studies read Open-Meteo, a service that re-serves other
centres' forecasts through one API, and the production service does not.

**The two studies of past weather score six products over the trial area's metered generators.**
The six are the Copernicus Atmosphere Monitoring Service's satellite irradiance (CAMS), ECMWF's
ERA5 reanalysis, three runs of the German weather service's (DWD's) ICON model — the 2 km
central-Europe nest ICON-D2, the 6.5 km Europe nest ICON-EU, and ICON global — and the Met Office's
2 km UK model (UKV). A reanalysis re-runs a weather model over the past, pulled towards
observations. [Which weather product best describes past
sunshine?](../studies/weather-products-for-the-past.md) scores December 2022 to September 2026 and
finds CAMS best by a wide margin, and ICON-D2 the best of the weather models as served. [Which weather
product best describes past wind?](../studies/weather-products-for-past-wind.md) scores August 2024
to September 2026 and finds UKV and ICON-D2 best. CAMS publishes no wind, so the wind study scores
the other five.

**Each product in the tables below carries two separate facts:**

- **Roadmap status** — copied word for word from the product's row in the [Weather
  data](../roadmap/data-sources.md#weather-data) catalogue, symbol and qualifier included: ✅
  ingested today, 🚧 planned, 🔬 research. A product with no catalogue row reads "Not on the
  roadmap".
- **Scored in** — the study pages that score the product, or "Not scored".
...
below say which. Global irradiance is all the sunlight reaching a horizontal surface; the direct
beam is the part arriving straight from the sun, and diffuse is the rest, scattered by cloud and
air. A solar-farm model needs the split because tilted panels catch the two parts differently. Five
...
- **Start dates are the first hour with a non-null value** at Lincoln (53.23°N, 0.54°W), queried on
  ...
  western boundary near 2.5°W, leaving South West England and South Wales outside it ([Which weather
  product best describes past wind?](../studies/weather-products-for-past-wind.md)).
- **Latency is the time from a run's initialisation to the whole run being available on
  ...
  00 UTC for the 6-hourly models, a later run for AROME, DMI, and KNMI. Each figure is one run, not
  an average.
...
- **The Licence column gives the licence of the data.** Open-Meteo's free API is for non-commercial
  ...
  plan.
- **"Period-ending" describes the meters**, which label each half-hourly reading with the time the
  half-hour ends. A product stamped at the middle of its averaging period needs shifting to match.

## Which archives keep whole past forecast runs

**Issue 810 compares models at the same lead time, so it needs an archive that keeps whole past
runs or serves a fixed lead.** A model run every 3 hours and a model run every 6 hours reach a
given hour at different ages, so a series that always takes the freshest run compares delivery
cadence as well as forecast quality. Open-Meteo publishes three archives of the same forecasts, and
Dynamical.org publishes a fourth kind:
...
- **The Previous Runs archive serves each variable at a fixed offset of 1 to 7 days**
  ...
  the earliest, and at day 1 only for UKV, ICON-D2, and the HARMONIE-AROME and AROME models (table
  below).
- **The Single Runs archive keeps whole runs** ([Single Runs
  ...
  forecasts re-run after the fact with the model version of that cycle. The March to June 2024 runs
  sampled are 00 and 12 UTC only, and 06 UTC runs are present from 2024-08-10. Runs from 2026-05-12
  06 UTC use IFS Cycle 50R1.
- **Dynamical.org keeps whole runs as Icechunk (Zarr) repositories**, listed in its STAC catalogue.
  The start dates below come from Dynamical.org's catalogue pages, checked against each store's
  `init_time` coordinate where the date matters here. The longest reach back to 2020-10-01 (GEFS)
  and 2021-05-01 (GFS). Every catalogue page states that "data.dynamical.org access ends September
  30, 2026", so a URL on that host has to move to a supported access pattern.
...
only. A lead-matched comparison with UKV, ICON-D2, or a HARMONIE-AROME or AROME model as one arm is
...
**Of the forecast models in the tables below, only ECMWF's IFS 9 km forecasts, GFS, the Met Office
global model, ICON-EU, and WeatherNext 3 serve a producer's own direct or diffuse field.** For the
other models in these tables, Open-Meteo makes the split by separation, except for DMI
HARMONIE-AROME, whose served direct field is probably not a direct beam at all (note below the
first table).
UKV, ICON-D2, and ICON global, which the studies score, also serve the producer's own direct field,
as do UKV and MOGREPS-UK on the Met Office's own feed — see [which sources carry which irradiance
components](../roadmap/data-sources.md#which-sources-carry-which-irradiance-components). Among the
reanalyses surveyed here, CERRA, COSMO-R6G2, and ICON-DREAM-EU carry the producer's own direct
field, as ERA5, already scored, does.

**Since IFS Cycle 50r1 went live on 12 May 2026, ECMWF no longer runs a separate "HRES" forecast:
the single 9 km forecast is the ENS control member**
...
Open-Meteo still calls the product "IFS HRES 9 km", and so does this page.

"Grid" is the producer's native grid spacing, with the serving grid in brackets where the two
differ. "Past runs" names the archive that keeps whole runs or a fixed lead, from the section above.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **ECMWF IFS ENS** (Dynamical.org) | [✅](../roadmap/data-sources.md#weather-data) | Not scored | ECMWF, 51-member physics ensemble | 9 km (0.25° served) | Yes (global) | 2024-04-01 to present; Dynamical.org is back-filling from MARS to 2016-03-08 | Global only (`ssrd`) | 10, 100 m | 4 a day from ECMWF, Dynamical.org keeps 00 UTC; 360 h; 3-hourly to 144 h, then 6-hourly | Not established | Whole runs | Icechunk (Zarr) | CC BY 4.0 and ECMWF terms | [Dynamical.org](https://dynamical.org/catalog/ecmwf-ifs-ens-forecast-15-day-0-25-degree/) |
| **ECMWF IFS ENS 9 km Europe** (Open-Meteo, `ecmwf_ifs_europe_ensemble`) | Not on the roadmap (the catalogue's ENS row is the Dynamical.org feed) | Not scored | ECMWF, 51-member physics ensemble, European domain | 9 km, O1280 (served natively) | Data at Lincoln | 2026-09-23 01 UTC at Lincoln, asking for 7 past days on 2026-09-23 | Global; direct native (ECMWF's `fdir`, per Open-Meteo's downloader) | Not checked | 00 and 06 UTC; 15 days; hourly to 90 h, 3-hourly to 144 h, then 6-hourly | Not established | None beyond the latest runs | Open-Meteo Ensemble API | CC BY 4.0 (ECMWF Real-time Catalogue) | [Open-Meteo](https://open-meteo.com/en/docs/ensemble-api), [downloader](https://github.com/open-meteo/open-meteo/blob/main/Sources/App/EcmwfEcpds/EcmwfEcpdsVariable.swift) |
| **ECMWF IFS HRES 9 km** (`ecmwf_ifs`; the ENS control since 12 May 2026) | Not on the roadmap | Not scored | ECMWF, global physics model | 9 km (served natively) | Yes (global) | 2017-01-01, complete to 2026-09-21 | Global; direct native (ECMWF's `fdir`, per Open-Meteo's downloader); diffuse = global − direct | 10, 80, 100 m | 00 and 12 UTC to 15 days, 06 and 18 UTC to 144 h; hourly to 90 h, 3-hourly to 144 h, then 6-hourly | 7.0 h (Open-Meteo) | Single Runs from 2024-03-14; Previous Runs from 2025-10-01 | Open-Meteo | CC BY 4.0; ECMWF's Real-time Catalogue has been CC BY 4.0 since 2025-10-01 | [Open-Meteo](https://open-meteo.com/en/docs/ecmwf-api), [downloader](https://github.com/open-meteo/open-meteo/blob/main/Sources/App/EcmwfEcpds/EcmwfEcpdsVariable.swift), [ECMWF](https://www.ecmwf.int/en/about/media-centre/news/2025/ecmwf-makes-its-entire-real-time-catalogue-open-all) |
...
| **ECMWF AIFS-ENS** | [🔬 (v2.1, uncertain)](../roadmap/data-sources.md#weather-data) | Not scored | ECMWF, machine-learned ensemble | 0.25° | Yes (global) | 2025-07-02 | Global only (`ssrd`) | 10, 100 m | 4 a day; 360 h; 6-hourly | Not established | Whole runs | Icechunk (Zarr) | CC BY 4.0 and ECMWF terms | [Dynamical.org](https://dynamical.org/catalog/ecmwf-aifs-ens-forecast/) |
| **DWD ICON-EU** (Dynamical.org) | [🔬 (v0.9, uncertain)](../roadmap/data-sources.md#weather-data) | Both studies, through Open-Meteo's copy ([sunshine](../studies/weather-products-for-the-past.md), [wind](../studies/weather-products-for-past-wind.md)) | DWD, regional physics model | 6.5 km (0.0625° served) | Yes | 2026-02-10 | Direct and diffuse native | 10 m only in Dynamical.org's store; Open-Meteo serves 80 and 120 m | Every 3 hours on Open-Meteo, Dynamical.org keeps 00, 06, 12, and 18 UTC; 120 h; hourly to 78 h | Not established | Whole runs (Dynamical.org); Previous Runs from 2024-01-19 (Open-Meteo) | Icechunk (Zarr); Open-Meteo | CC BY 4.0 | [Dynamical.org](https://dynamical.org/catalog/dwd-icon-eu-forecast-5-day/), [Open-Meteo](https://open-meteo.com/en/docs/dwd-api) |
...
| **DMI HARMONIE-AROME DINI** | Not on the roadmap | Not scored | The Danish Meteorological Institute (DMI) for the UWC-West consortium; HARMONIE-AROME is the convection-permitting regional model several European weather services share | 2 km (Open-Meteo's figure) | Yes (39.7–62.7°N, 25.4°W–40.1°E) | 2024-06-28 | Global; a field Open-Meteo serves as direct, probably not DMI's direct beam (note below) | 10, 50, 100, 150, 250, 350, 450 m (Open-Meteo interpolates 80 m) | 8 a day; 2.5 days | 2.8 h (Open-Meteo, 06 UTC run) | Previous Runs from 2024-06-29 | Open-Meteo | CC BY 4.0 | [Open-Meteo](https://open-meteo.com/en/docs/dmi-api), [domain](https://api.open-meteo.com/data/dmi_harmonie_arome_europe/static/meta.json), [downloader](https://github.com/open-meteo/open-meteo/blob/main/Sources/App/Dmi/DmiDownloader.swift) |
...
| **Google WeatherNext 3** | [🔬 (after v2, unlikely)](../roadmap/data-sources.md#weather-data) | Not scored | Google DeepMind, machine-learned 64-member ensemble | 0.1° | Yes (global) | 2026-01-01, with 2024 and 2025 being back-filled | Global; direct native (`fdir`); diffuse = global − direct | Not checked | 4 a day to 15 days, plus hourly interim runs to 48 h; hourly | Not established | Not established | Access request; members only in the Cloud Storage Zarr store | CC BY 4.0 once at least 1 h old; real-time data under Google DeepMind's experimental terms | [model specs](https://developers.google.com/weathernext/guides/models), [access](https://developers.google.com/weathernext/guides/access-forecast) |
...
field unused, with a comment that the latter "seems to be DNI". The mapping has not been checked
against DMI's own files.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **CERRA** | [🔬 (deprioritised)](../roadmap/data-sources.md#weather-data) | Not scored | Copernicus regional reanalysis, HARMONIE-ALADIN with 3D-Var, forced by ERA5 | 5.5 km, 1069 × 1069 points | Yes (European domain) | 1984-09-01 to 2026-06-30 on the CDS on 2026-09-23 | Global and direct native, forecast fields only, accumulated | 10 m; 15, 30, 50, 75, 100, 150 to 500 m | 3-hourly analyses; hourly values from forecast leads 1–3 h | About 12 weeks on 2026-09-23 (the catalogue row gives ~3.5 months) | Copernicus CDS; no area cropping, so every field is the whole domain | CC BY 4.0 | [CDS](https://cds.climate.copernicus.eu/datasets/reanalysis-cerra-single-levels), [user guide](https://confluence.ecmwf.int/x/WFQ7E), [Ridal et al. (2024)](https://doi.org/10.1002/qj.4764) |
...
direct beam improves its score by 0.03 to 0.11 points. A place in this ranking is not a decision to
score the product, and no extension of the study has been agreed. The ranking, with each product's
roadmap status and one reason:
...
1. **ECMWF IFS HRES 9 km on Open-Meteo** (not on the roadmap) — a 9 km forecast from the IFS model
   ...
   API](https://open-meteo.com/en/docs/historical-weather-api)), and its served lead is
   undocumented, so a study using it has to say both.
2. **CERRA** (🔬 deprioritised) — a regional reanalysis at 5.5 km with a native direct field, hourly
   through forecast leads 1 to 3. Its latency is why the catalogue deprioritises CERRA for the live
   service, and is no obstacle for a study of past weather. The download volume is the obstacle:
   every field is the whole European domain.
3. **COSMO-R6G2** (not on the roadmap) — a third regional-reanalysis family at ~6 km with hourly
   global and direct irradiance, as plain monthly NetCDF. The archive stops at December 2025, nine
   months short of the study's September 2026 end, and arrives in yearly batches, so COSMO-R6G2 can
   only be scored on a shorter window. The hourly means are stamped at :30 and need shifting 30
   minutes to be period-ending.
4. **ICON-DREAM-EU** (not on the roadmap) — an ICON reanalysis at 6.5 km with native direct and
   diffuse, which sets an analysis against the short-range ICON-EU forecast already scored inside
   one model family.
...
wind close to hub height at 3 to 5.5 km.** Neither covers the whole study window: CERRA ends on
2026-06-30 and NORA3 on 2026-08-31, against the study's September 2026 end. The wind farms' hub
heights are not known; the study scores 80 m and 100 m wind. A place in this ranking is not a
...
1. **CERRA** (🔬 deprioritised) — wind at 75 m and 100 m, bracketing 80 m, at 5.5 km, hourly, to
   June 2026. The download volume is the same obstacle as for sunshine.
2. **NORA3** (not on the roadmap) — HARMONIE-AROME at 3 km with wind at 50 m and 100 m, hourly, to
   August 2026, over OPeNDAP, so a point extraction is cheap. The components are grid-relative and
   need rotating.
3. **COSMO-R6G2** (not on the roadmap) — 100 m wind, hourly, at ~6 km, to December 2025.
4. **ECMWF IFS HRES 9 km** (not on the roadmap) — 10, 80, and 100 m wind from 2017, through the
   existing Open-Meteo interface.
5. **DMI HARMONIE-AROME DINI** (not on the roadmap) — 100 m wind at 2 km from 2024-06-28, from a
   different consortium than UKV and ICON-D2. The 80 m wind Open-Meteo serves is interpolated.
...
Previous Runs puts ICON, UKV, and the HARMONIE-AROME models at a fixed lead, and for those models at
day 1 only.** Issue 810 proposes ECMWF ENS via Dynamical.org, the product in production, as the
incumbent each other model is compared against. A place in this list is not a decision to score the
product. Ordered by how far back whole runs reach:
...
2. **Dynamical.org GFS** (not on the roadmap), from 2021-05-01 — four runs a day to 384 h with
   global irradiance and wind at 10, 80, and 100 m.
...
4. **Open-Meteo Single Runs of ECMWF IFS HRES 9 km** (not on the roadmap), from 2024-03-14 — the one
   whole-run archive here with a native direct beam, but Open-Meteo calls the early runs hindcasts,
   the runs sampled from early 2024 are 00 and 12 UTC only, and the model changes to Cycle 50R1 on
   2026-05-12.
...
6. **Open-Meteo Previous Runs**, from 2024-01-19 at the earliest — the only route surveyed to ICON,
   UKV, AROME, and the HARMONIE-AROME models at a fixed lead, capped at the offsets in the table
   above.
