...
about 6 weeks at the time of writing.** The service produces a probabilistic power forecast for each
of the 31 time series currently in scope, half-hourly out to 14 days ahead, refreshed every 6 hours,
and writes every forecast to the `power_forecasts` table on S3. The AWS running cost is estimated at
£25 to £35 a month.

## What the service has done so far

**Version 0.1 produced a forecast for every 6-hourly run it was scheduled for.** Between the 18:00
UTC slot on 15 July 2026 and the 18:00 UTC slot on 13 August 2026 the schedule called for 117
consecutive slots, and all 117 delivered forecasts for all 28 time series then in scope. The count
was verified by reading the forecast table back off storage rather than from Dagster's own record:
...
**The one input failure in the version 0.1 window degraded the forecast instead of stopping it,
which is what the project's first design principle requires: the power forecast never stops.**
Dynamical.org first published the 2026-08-09 00Z run of the European Centre for Medium-Range Weather
Forecasts (ECMWF) ensemble with one weather variable wholly missing, and repaired the run 3 hours 25
minutes later. Version 0.1 treated a wholly-missing variable as fatal for the weather-ingestion
step, so no weather data was stored for 9 August. Despite that, all four affected forecast slots ran
on the previous day's weather run instead, at 36 to 54 hours old against the 12 to 30 hours a
healthy slot sees. No forecast slot was missed, the degradation was bounded, and the degradation was
reconstructable afterwards because every forecast row records which weather run produced it.
Restoring the missing run by hand is the only human intervention the service has needed, and it took
about a minute.
...
time series — three more than version 0.1, under a newly promoted model. From that first slot to the
06:00 UTC slot on 28 August 2026 the schedule called for 58 consecutive slots, and all 58 produced a
forecast for all 31 time series. No weather run was missed in that window: every slot forecast from
weather data between 12 and 30 hours old, which is the healthy band for a once-daily run. No human
...
**Version 0.2 adds the telemetry version 0.1 lacked.** An automated check now reads each slot's
forecast rows back and reports how many weather runs were missed, so a slot forecasting from stale
inputs is recorded as degraded rather than passing unremarked. A wholly-missing weather variable is
retried for 4 hours rather than treated as fatal, which is what would have made the 9 August
intervention unnecessary. And the deployment reports errors to Sentry, which the version 0.1
deployment never did. One gap is open and tracked: a slot that is merely degraded, with nothing
having actually failed, still reports itself healthy to the check-in monitor, so the degradation
reaches the Dagster checks page rather than an operator.

## The forecast itself is still a "dummy" placeholder
...
whatever was most recently trained, and a change of "champion" model is one deliberate act rather
than a redeployment. Continuous integration runs the linters, the type checker, and the test suite
on every code change. The intervention log records every occasion a human had to touch the running
service, because that measurement cannot be reconstructed after the fact.
