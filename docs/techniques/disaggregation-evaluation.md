...
## Spoke 7: A simulated world built from frozen physics models (v2)

Fit the per-site [differentiable-physics](differentiable-physics.md) modules on real telemetry,
freeze their parameters, run them forward on real weather, and sum the sites into a simulated
substation. Every quantity the disaggregator must recover — each site's generation, the residual
demand, each fitted capacity — is written down by construction, and a whole fleet of simulated
substations costs no metering. Simple edits extend the same trick to the other two label-poor
problems: move a known fraction of one substation's power onto a named neighbour over a known
window to label a switching event, or step a generator's capacity down on a known date.

**Caveat**: this spoke is circular in a way the others are not. An estimator scored on data
generated by its own model family measures whether the parameters are identifiable, not whether
the model matches reality, and the simulated telemetry carries none of the meter noise, false
zeros, or apparent-power bounce that real telemetry does. A failure here is decisive; a pass only
establishes the best case. Where an edit can be applied to real telemetry instead — injection
adds to a real series rather than replacing it — prefer the real-data version, and reserve the
fully simulated world for the labels no edit to real data can produce, such as removing the
unmetered fleet entirely.
