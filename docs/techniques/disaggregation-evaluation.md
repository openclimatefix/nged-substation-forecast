...
a simulated substation. Every quantity the disaggregator must recover is written down by
construction — each site's generation, the residual demand, and each fitted capacity — and a whole
fleet of simulated substations costs no metering. Unlike Spoke 1, the simulated sum covers the
unmetered fleet and the demand as well as the metered sites.
...
A good method scores well across all eight spokes despite their differing biases. A method has
overfit to the easy case if it scores well on synthetic aggregation (Spoke 1) but fails
physical-consistency checks (Spoke 3) on real data. The leaderboard columns for disaggregation are
not "the metric" — they are these spokes. Because labels are weak, the protocol must be more
carefully reasoned and transparently caveated than a standard forecasting evaluation: "no clean
ground truth" must not slide into "any evaluation will do."
