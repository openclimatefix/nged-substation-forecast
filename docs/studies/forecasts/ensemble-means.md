...
an XGBoost model given ICON-D2-EPS's ensemble mean had the lowest power error of the four
ensemble-mean products that Open-Meteo serves.** The other three products are the ensemble means of
MOGREPS-UK, ICON-EU-EPS, and ECMWF IFS ENS at 0.25 degrees. Every error on this page is a mean
absolute error as a percentage of each generator's effective capacity. For solar power, ICON-D2-EPS's
...
mean gave 5.75% and ECMWF ENS's mean 5.79%, a gap of 0.04 points that this page does not treat as a
difference. The ERA5 reanalysis gave 6.28%, ICON-EU-EPS's mean 6.72%, and no weather 12.60%.
...
![Figure 1: Of the ensemble means tested, ICON-D2-EPS's had the lowest error for solar power and, by
a small margin, for wind power](../assets/ensemble_means_mae.svg)

> **How this page was made.** The research question came from a human. Everything else — the code
> behind every result, the analysis, the figures, and the text — was written by Claude, Anthropic's
> AI model (for this page, Claude Sonnet 5). Several independent Claude reviewers have checked the
...
domain. All four Open-Meteo ensemble means scored 2.5 to 3.8 points worse than CAMS and 3.9 to 5.2
points better than no weather at all. CAMS is a satellite retrieval available only after the hour, so it
sets the target that a forecast of past sunshine could reach and is not a competing forecast.

**For a wind model, read a 100 m wind speed from ICON-D2-EPS's mean if the product is available, and
otherwise ECMWF ENS's.** Given the 100 m and 10 m speeds, ICON-D2-EPS's mean scored 5.08%, against
5.71% for ECMWF ENS's mean and 6.11% for ERA5, and it beat ERA5 in all 12 held-out weeks. Open-Meteo
serves no 100 m wind for MOGREPS-UK's or ICON-EU-EPS's mean, so neither could be scored at hub
height. At 10 m, ICON-EU-EPS's mean scored 0.45 points worse than ERA5 in both settings, and does not
suit a wind model.

**These verdicts would change with a longer window or a lead-resolved comparison.** The window holds
one summer, and none of its wind is from autumn or winter. The ensemble means are
stitched series that carry no run time, so the page says nothing about which product forecasts best
at a stated lead such as day-ahead.
...
meter reads exactly zero while ERA5 reports more than 100 W/m². The wind rows drop every hour holding
...
mean of the 51 members, the newest run for each valid time from leads 3 to 69 hours, radiation held
...
product's wind speed at 10 m. The hub-height design reads the 100 m and the 10 m speeds, for the four
...
ensemble on different grids and with different stitching and interpolation, so the gap measures the
pipeline and not the ensemble.
...
**The ensemble means are stitched series.** They carry no run time, so their leads are short and
mixed. The page says nothing about day-ahead skill or about how the error grows with lead.

**Products differ in coverage.** ICON-D2 does not cover South West England or South Wales, and every
generator here lies inside its domain. MOGREPS-UK's and ICON-EU-EPS's means have no 100 m wind.

**The capacity denominator is the latest effective capacity per generator in `data/effective_capacity`
...
and takes about 15 minutes on the CPU. The script refuses to overwrite, so move the existing files to
