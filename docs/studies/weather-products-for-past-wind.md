...
describe past wind best.** Across the window both beat the ERA5 reanalysis by about half a point of
capacity, but from October to March only ICON-D2's advantage is detectable. ICON-D2 and UKV cannot
be separated robustly. **ICON-EU, the European ICON model, beats the reanalysis at either height it
is shown.** **ICON global is the weakest product, and about half of its gap is a pair of steps in
the wind it is served at one generator.** Every error on this page is a mean absolute error as a
...
comparison has to state the height and the lead each product is scored at.** CAMS, the Copernicus
Atmosphere Monitoring Service's satellite retrieval, describes past sunshine best but publishes no
wind, so it has no arm here.
...
- **Hub heights.** The three wind farms' hub heights are not known. Open-Meteo serves each ICON
  ...
  their 100 m wind. A check arm shows UKV its 80 m wind, which scores 0.02 points worse than its 100
  m wind.
...
- **History.** UKV's hub-height wind starts on 12 August 2024, when Open-Meteo's own UKV downloader
  started, which is why the comparison covers only 2 years. ERA5 starts in 1940, and the ICON
  products' 80 m wind is complete from December 2022.

## How the comparison was made

**The method is the solar study's, with three differences: the power hour is centred on the label,
hours holding an exactly-zero half-hour are dropped, and every product is read from its nearest land
grid cell.**
...
- **Same rows, same model, same number of columns.** An hour is kept only if all five products cover
  ...
  which side of the UKV upgrade the hour falls on. Every product is read from its nearest land grid
  cell.
...
since its upgrade: 0.79 points, against 0.53 on the same calendar months before it. Across the
...
**ICON-EU beats ERA5 by 0.31 points [0.17, 0.45] when shown its native 80 m wind, and by 0.22 points
[0.08, 0.35] when shown the served 100 m wind the plan specified before the first run.** Both arms
...
April to September ICON-EU is 0.48 points ahead, and from October to March the two cannot be told
apart. ICON-EU is 0.13 points behind UKV [0.01, 0.23]. That interval includes zero under the second
hyperparameter setting and with UKV at 80 m, and the two cannot be told apart before the upgrade or
in winter. Since the upgrade UKV is 0.41 points ahead of ICON-EU [0.33, 0.49].
...
the hours where the two leads are equal ICON global is 0.63 points behind [0.41, 0.86], against 0.76
on the others. At the two generators without the steps ICON global is still 0.33 and 0.40 points
behind, which is consistent with its coarser grid, though this study does not separate the grid from
other differences between the two models. At one generator the nearest ICON global grid cell is
influenced by the sea; every figure here reads the nearest land cell instead.
...
- **Historical features in the live service: UKV, or ICON-D2 where ICON-D2 reaches.** UKV covers all
  ...
  arrives about 5 days late and cannot supply the last few hours at run time.
- **Training history: ICON-D2 where it covers, from December 2022, with ICON-EU or ERA5 elsewhere.
  ...
  archives before August 2024 have not been screened for steps like the pair in ICON global's served
  wind.
...
- **Terrain.** [Issue #826](https://github.com/openclimatefix/nged-substation-forecast/issues/826)
  expected resolution to matter most for wind because terrain shapes hub-height wind. The three
  generators are onshore in flat country, and two of them share one ERA5 grid point, so this study
