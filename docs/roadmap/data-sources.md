...
| **CAMS solar radiation** (Copernicus Atmosphere Monitoring Service) | 🔬 (offline only, uncertain) | Worth *considering* as an alternative or a companion to SARAH-3, on the same offline-only footing. Carries **global, direct, diffuse, and direct-normal** irradiance under both clear sky and observed cloud, from 2004-02, under CC-BY. The **sub-hourly** steps — down to 1 minute — are the draw: they are what the [dynamic thermal model](../techniques/differentiable-physics.md) needs and what SARAH-3's 30 minutes cannot supply. The catch is [how CAMS is delivered](#cams-arrives-as-point-time-series-not-a-grid). |
