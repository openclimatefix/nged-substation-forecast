...
| **CM SAF** (Satellite Application Facility on Climate Monitoring) | 🚧 (v0.7) | High-resolution satellite-derived irradiance, used to estimate **solar PV** capacity. SARAH-3 provides global (SIS), **direct (SID) and direct-normal (DNI)** irradiance at 0.05° / 30-minute resolution from 1983 (diffuse = SIS − SID) — the beam/diffuse split the [DP solar model](../techniques/differentiable-physics.md#the-core-building-block-differentiablesolarplant) needs, at a resolution matching the half-hourly metering. | Two operational facts to confirm before v0.7 leans on SARAH-3. Its near-real-time **Interim Climate Data Record** latency is reported inconsistently — 5 days on the CM SAF product page, 2 to 3 days in [Pfeifroth et al. (2024)](https://doi.org/10.5194/essd-16-5243-2024) — so check the Web User Interface directly. And Pfeifroth et al. report the accuracy target met everywhere except winter **above 55°N**, which barely touches NGED: of the 3,399 connected generators in the August 2026 Embedded Capacity Register carrying a usable northing, 6 sit above 55°N.
| **CAMS solar radiation** (Copernicus Atmosphere Monitoring Service) | 🔬 (v0.7, uncertain) | Satellite-derived irradiance worth racing against SARAH-3 before v0.7 settles on one. Carries **global, direct, diffuse, and direct-normal** irradiance under both clear sky and observed cloud, at time steps of 1 minute, 15 minutes, 1 hour, 1 day, or 1 month, under CC-BY. Freshness is its strength: the [Atmosphere Data Store catalogue](https://ads.atmosphere.copernicus.eu/datasets/cams-solar-radiation-timeseries) offered data from 2004-02 up to **the previous day** when queried on 2026-08-24, against SARAH-3's days-to-weeks. Its **sub-hourly** steps are also the input the [dynamic thermal model](../techniques/differentiable-physics.md) needs and SARAH-3's 30 minutes cannot supply. What counts against it: **each request returns one point, not a grid** — the request form takes a single latitude and longitude — so ~2,500 substations means ~2,500 requests against a per-user daily cap, which fights the H3 pipeline every other source rides. A gridded CAMS radiation dataset exists but lags roughly 6 months. The all-sky product is limited to the Meteosat Second Generation and Himawari fields of view; Great Britain sits inside, but the same low-winter-sun degradation SARAH-3 documents applies. |
