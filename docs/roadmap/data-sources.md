...
| **WeatherNext 3** (Google DeepMind) | 🔬 (after v2, unlikely) | Carries total-sky **direct** short-wave (`fdir`) alongside global short-wave, so it is the one ensemble in this table offering [the PV forward model](disaggregation.md#the-forward-model) a beam/diffuse split; ICON-EU carries the split too, and sooner, but as a single deterministic run stopping at 120 hours. A machine-learned 64-member ensemble, single-level fields at 0.1° on hourly steps, running 15 days from the 00/06/12/18 UTC cycles and 48 hours from the hourly interim runs; 2 m temperature and dewpoint also arrive at 0.05° from a head trained directly on in-situ surface observations from airport stations, regional station networks, ships, and buoys ([Rasp et al. (2026)](https://arxiv.org/abs/2609.03582); [model specs](https://developers.google.com/weathernext/guides/models)). The paper reports lower CRPS than ECMWF ENS on surface solar radiation, on the 6-hour accumulations rather than the hourly fields, but every radiation evaluation in the paper scores against an ECMWF analysis and none scores against a surface measurement — so any irradiance gain would have to be measured downstream on power. Two structural differences from AIFS-ENS: all 64 members start from one analysis, so the spread is model spread with no initial-condition spread, and no member pairs with an ECMWF ENS member. Data at least 1 hour old is CC-BY-4.0, real-time data falls under Google DeepMind's experimental terms, and access needs [a request form](https://developers.google.com/weathernext/guides/access-forecast). BigQuery and Earth Engine serve six summary statistics per variable (the mean, and the 10th, 25th, 50th, 75th, and 90th percentiles); the 64 members sit only in the Cloud Storage Zarr store. A worse folds problem than ICON-EU or AIFS-ENS — the archive starts 2026-01-01, with 2024 and 2025 being backfilled — and on current priorities we do not expect to reach this source inside the Network Innovation Allowance project. |
