...
| **AIFS-ENS** (ECMWF) | 🔬 (v2, uncertain) | ECMWF's machine-learned ensemble, operational since 1 July 2025 with the same 51 members, 6-hourly steps and 15-day horizon as the physics ensemble, and more accurate than it on the majority of variables and lead times ([Lang et al. (2026)](https://doi.org/10.1038/s44387-026-00073-7)). Whether that translates into a better substation-load forecast is an open question, and the swap is cheap to test because the two ensembles share a shape. Same folds problem as ICON-EU: the archive starts mid-2025, so it is an ad-hoc ablation before it is a canonical source. |
