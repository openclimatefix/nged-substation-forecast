...
**Two ideas outlive Flexpectation, and the capacity map is the more valuable.** The other is [an irradiance nowcast](#an-irradiance-nowcast-would-be-a-more-useful-product): recovering irradiance from the calibrated fleet is mostly a check on the capacity estimates rather than a second product, and the nowcast is the one exception that would stand on its own.
...
**Sheffield Solar's PV Live already publishes that estimate for Great Britain's embedded PV.** National Grid's Electricity System Operator (ESO) funded Sheffield Solar's original PV Live methodology. ESO, now the National Energy System Operator (NESO), has procured PV Live as a commercial service since 2021. PV Live models yield from a live sample of reporting PV systems, then scales that sample up to each grid supply point and to the nation using an estimate of installed capacity. A second national estimate that disagreed with PV Live would leave every user to arbitrate between the two estimates, and would duplicate a service the system operator already depends on.
...
**Huxley et al. name three ways past the capacity registers, and the graph-structured engine's design draws on two of them.** Huxley et al. call for a move from static registers towards an estimate of operational grid-connected capacity. Huxley et al. suggest that estimate could come from power flows on the electricity network, from satellite imagery, or — the route they judge more likely to succeed — from a combination of complementary datasets. [The graph-structured engine](#the-graph-structured-engine) fits capacity to power flows on the electricity network, the same route as [the nearest GB precedent we found](#what-already-exists-prior-art): UK Power Networks' Power Flow to Solar Capacity. [The capacity work](../techniques/disaggregation-evaluation.md#spoke-4-cross-source-corroboration-label-free-indirect) also folds registered capacity in as a prior on that fit, so the design draws on Huxley et al.'s combination route too.
