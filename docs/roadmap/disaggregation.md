...
## Correcting satellite irradiance over Great Britain (v2)

PV is the DER we have the best chance of disaggregating well, and error we leave in the PV estimate does not stay there — the framework absorbs it into gross demand or into another DER. So the irradiance input is worth improving. What follows is what we know before starting, so that v2 does not spend a month rediscovering it.

**Check first whether CAMS publishes per-timestep uncertainty, before building anything.** The CAMS Radiation Service has a probabilistic error model that conditions the distribution of its own deviations on solar zenith angle, clear-sky index, and cloud probability, reporting an average continuous ranked probability score of 50 W/m² and strong robustness to which stations trained it ([Lezaca Galeano et al. (2025)](https://doi.org/10.1002/solr.202500568), [Schroedter-Homscheidt et al. (2026)](https://meetingorganizer.copernicus.org/EMS2026/EMS2026-533.html)). Both British research-grade stations, Camborne and Lerwick, are in its 66-station training set, so Great Britain is represented at either end of its latitude range. As of September 2026 the model is a concept study rather than part of the operational chain, and the request form exposes no uncertainty variable. **If it still is not published when v2 starts, ask the paper's corresponding author whether they can share the uncertainty estimates, even as a one-off file transfer** — a look-up table is small, and the worst case is being told no.

**Do not start by correcting aerosols.** CAMS tested aerosol and albedo properties as conditioning variables and dropped them, because their contribution to the deviation ranked below cloud properties and irradiance level. The service already uses 3-hourly aerosol analyses rather than a monthly climatology, so the aerosol term is both smaller than the cloud term and largely handled.

**Do not interpolate raw residuals between ground stations.** Satellite irradiance beats irradiance interpolated from a ground station once that station is more than about 25 to 34 km away ([Zelenka et al. (1999)](https://doi.org/10.1007/s007040050015)). The Met Office's [MIDAS](https://catalogue.ceda.ac.uk/uuid/76e54f87291c4cd98c793e37524dc98e/) network has on the order of 50 stations reporting hourly radiation across Great Britain's 229,000 km², which puts the average point about 34 km from its nearest station — on the break-even line. Cloud residuals decorrelate over tens of kilometres, so interpolating them mostly spreads one station's local cloud-timing error across a cell 40 km away. A pooled regression of the residual on clear-sky index, solar zenith angle, season, and distance to coast is the formulation that survives this station density, and it is what CAMS has already fitted.

**Some of the satellite-minus-station residual should deliberately not be corrected out.** The CAMS authors are explicit that their modelled deviation is not a difference from ground truth, but the expected spread between a spatial average over several km² and a point measurement. Which of those we want flips between milestones: a single metered solar farm in [v0.7](capacity-estimation.md#irradiance-inputs) occupies under a tenth of a grid cell and behaves like a point, whereas an unmetered domestic fleet spread across a substation's catchment is closer to the area average the satellite already reports. Correcting the grid towards a pyranometer would improve the first case and degrade the second.

**Per-timestep uncertainty is worth more to us than a corrected mean.** A variance that widens under broken cloud and narrows under clear skies feeds a heteroscedastic likelihood in the [reconstruction loss](#the-forward-model), which is the [probabilistic treatment](../techniques/probabilistic-forecasting.md) the rest of the system already assumes. A corrected mean with no uncertainty tells the optimiser nothing about which timestamps to trust.

**If any of this is published, publish a dataset with an evaluation report, not a method.** Merging a ground network into a satellite field at national scale was done for Belgium by [Journée and Bertrand (2010)](https://doi.org/10.1016/j.rse.2010.06.010), and site adaptation is routine in the solar industry, so a novelty claim would not survive review. Validation would have to be leave-one-region-out rather than leave-one-station-out, hold Camborne and Lerwick out entirely, and beat a per-station monthly scale factor — and the end-to-end test is held-out metered PV, because the disaggregation's own residual always fits.
