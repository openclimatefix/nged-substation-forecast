...
gradients, same trees, and an identical early-stopping metric either way. The margin only starts to
help once the *link* is not the identity — a Poisson/Tweedie link (natural for a non-negative
quantity), where the margin lives in log-space so the correction becomes *multiplicative*, the loss is no longer
