...
item.** Most of what a naive count reads as missing weather is an artefact of the aggregation
rather than lost data. Without the renormalisation, one corrupt grid point nulls its *entire*
cell, because NaN propagates through a weighted sum, and a grid point feeds 4.92 cells on
average — so a very small amount of upstream corruption arrives looking like a great deal of
missingness. On 2025-06-04 00Z, the worst run in the archive by this measure, 0.014% of
`precipitation_surface` grid points would null **4,394** cells that way; renormalising leaves
...
So roughly 92% of the interior nulls this item appears to bound are aggregation artefacts rather
than missing weather, and what remains
