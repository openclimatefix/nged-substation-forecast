...
- Before committing to CM SAF as the irradiance source, race it against **CAMS** (the Copernicus Atmosphere Monitoring Service solar radiation time-series), because the two make opposite trade-offs ([data sources](data-sources.md#weather-data)). CM SAF's SARAH-3 ships as a 0.05° NetCDF grid reaching back to 1983, which suits the H3 pipeline and the pre-training history; CAMS reaches yesterday rather than days-to-weeks behind and offers steps down to 1 minute, but serves one point per request. The decision turns on two things worth measuring rather than arguing: how much the fitted plant model gains from sub-hourly irradiance, and whether ~2,500 point requests a day is workable against the CAMS request cap
