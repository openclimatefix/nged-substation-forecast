...
The fitted plant models are also reusable as a generator of labelled data: freeze their
parameters, run them forward on real weather, and every capacity the estimator must recover is
known by construction. That [v2 idea](index.md#v20-scale-up-future-research) supplements the
fault injection above rather than replacing it, because an estimator scored on data generated by
its own model family only demonstrates that the parameters are identifiable.
