...
*   **Data Cleaning (`packages/nged_json_data/src/nged_json_data/clean.py`):**
    *   Implement `clean_power_data(df: pl.DataFrame, variance_thresholds: dict[str, float] | None = None) -> pl.DataFrame`. This function will:
        1.  Sort by `end_time`.
        2.  Drop any rows where `value` is null.
        3.  Identify "stuck" sensors by calculating variance over a rolling window (e.g., 6 hours) instead of daily aggregates.
        4.  Nullify the `value` for specific time slices where the rolling variance falls below the substation's specific threshold, rather than dropping entire days. This preserves valid low-load periods.
        5.  The variance threshold must be configurable per substation (e.g., via a mapping of `time_series_id` to threshold). If a substation is not in the mapping, a sensible default or dynamic calculation should be used.
        6.  **Crucial:** Heavily document the rationale for this rolling variance threshold in the code comments, explaining *why* this approach is taken (to handle stuck sensors without losing valid low-load data). Ensure the function fails loudly (raises an exception) if *all* rows are removed by this filter.
...
*   **Delta Table Storage (`packages/nged_json_data/src/nged_json_data/storage.py`):**
    ...
    *   **Concurrency Control:** Implement a robust retry mechanism for concurrent writes in `append_to_delta` (e.g., using `tenacity` or a custom retry loop catching `DeltaProtocolError`), similar to the logic in `src/nged_substation_forecast/defs/nged_assets.py`. This is critical to prevent race conditions during parallel ingestion.
...
*   **Data Cleaning:** Empty/malformed input, rolling variance logic edge cases (exactly at threshold, slightly above, all rows removed, insufficient rows).
*   **Metadata Handling:** File corruption, conflict resolution, and concurrent access (mocking the file lock).
*   **Delta Table Storage:** Duplicate handling within the new data itself, correct partitioning, and concurrent write retries.
*   **General:** Ensure all components fail loudly on invalid data.

## Review Responses & Rejections

*   **FLAW-001 (Scientist):** ACCEPTED. The data cleaning logic has been redesigned to use a rolling window for variance calculation instead of daily aggregates, and it will nullify specific stuck time slices rather than dropping entire days.
*   **FLAW-002 (Scientist):** ACCEPTED. The `variance_threshold` will be made configurable per substation, rather than a global default, to account for different load profiles.
*   **FLAW-003 (Scientist):** ACCEPTED. A retry mechanism for concurrent writes will be implemented in `append_to_delta` to prevent race conditions during parallel ingestion.
