...
target_modules: ["packages/xgboost_forecaster/src/xgboost_forecaster/model.py", "packages/ml_core/src/ml_core/utils.py", "src/nged_substation_forecast/defs", "tests", "packages/xgboost_forecaster/tests"]
---
...
## 4. Remove `target_map` from `ml_core/utils.py` and its Callers
**Files:**
*   `packages/ml_core/src/ml_core/utils.py`
*   `src/nged_substation_forecast/defs/xgb_assets.py`
*   `src/nged_substation_forecast/defs/xgb_jobs.py`
*   `tests/test_xgboost_mlflow.py`

*   **Updates in `packages/ml_core/src/ml_core/utils.py`:**
    *   In `train_and_log_model`: Remove the extraction of `target_map` from `kwargs` and the `ValueError` raised if it is missing. Remove the logic that assigns `target_map` to the trainer.
    *   In `evaluate_and_save_model`: Remove the extraction of `target_map` from `kwargs`, the fallback to `forecaster.target_map`, and the `ValueError` raised if it is missing.
*   **Updates in Callers:**
    *   In `src/nged_substation_forecast/defs/xgb_assets.py` and `src/nged_substation_forecast/defs/xgb_jobs.py`: Ensure that `target_map` is not passed as an argument to `train_and_log_model` or `evaluate_and_save_model`.
    *   In `tests/test_xgboost_mlflow.py`: Remove the `target_map` argument from the `train_and_log_model` call (around line 324).
    *   Search for any other usages of `target_map` being passed to these functions in the `tests/` directory and remove them.
