...
- Add `substation_metadata: pt.DataFrame[SubstationMetadata]` to the function signature.
- Ensure the appropriate imports are added:
  ```python
  import patito as pt
...
- **Empty Data Guard:** Before performing any timestamp arithmetic, check `if actuals_30m.is_empty() or predictions.is_empty():`. If either is empty, log a warning and return an empty plot or skip plotting gracefully.
...
- After joining `latest_predictions` with `actuals_30m` to create `eval_df`, filter `eval_df` to the 14-day window:
  ...
  ```
- **Empty Plot Window Guard:** Add a check `if plot_df.is_empty():` after the 14-day filtering logic. If true, log a warning (e.g., "No data found in the 14-day window") and handle gracefully (e.g., return an empty plot or skip).
...
- Join `plot_df` with `substation_metadata` on `substation_number` explicitly using an `inner` join (e.g., `how="inner"`) to get `substation_name_in_location_table`.
...
**A. Verify Substation Names and Plot Configuration**
- The integration test already executes the full job, which includes the `substation_metadata` asset.
- Add assertions after the plot generation to read the generated HTML file and verify:
  - **Substation Name:** A known substation name is present in the output (e.g., substation `110375` is named "Woodland Way". Assert that `"Woodland Way"` is in the HTML content).
  - **Plot Title:** The presence of the `chosen_init_time` in the plot title.
  - **Independent Y-Axes:** The presence of `"resolve": {"scale": {"y": "independent"}}` in the Altair/Vega-Lite JSON configuration embedded in the HTML.
...
* **FLAW-003 (Tester):** ACCEPTED. Added explicit guards for empty `actuals_30m` and `predictions` DataFrames to prevent errors during timestamp arithmetic.
* **FLAW-004 (Tester):** ACCEPTED. Added a check for an empty `plot_df` after the 14-day filtering logic to prevent Altair from failing on empty data.
* **FLAW-005 (Tester):** ACCEPTED. Expanded integration test assertions to verify the plot title contains the `chosen_init_time` and the Altair configuration includes independent y-axis resolution.
* **FLAW-006 (Reviewer):** ACCEPTED. Updated the function signature to use `pt.DataFrame[SubstationMetadata]` for stricter typing and data contract enforcement.
* **FLAW-007 (Reviewer):** ACCEPTED. Explicitly specified `how="inner"` for the join between `plot_df` and `substation_metadata` to ensure clarity and predictable behavior.
