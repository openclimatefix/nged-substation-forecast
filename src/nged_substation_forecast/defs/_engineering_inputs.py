...
def load_engineering_inputs(
    ...
    **Memory: prune the NWP scan at the source.** The NWP Delta is large (tens of GB: every
    ...
    - ``init_time``: the table is partitioned by ``(nwp_model_id, init_time)``, so bounding
      ...
      prune partitions.
    - ``ensemble_member``: applied at the scan so we never decode the ~50 discarded members;
      ``delta_store.nwp`` writes one row group per member, so Parquet row-group stats skip the
      rest of each partition outright for this predicate, provided the predicate reaches the
      Parquet scan unchanged — which requires ``Nwp``'s declared ``ensemble_member`` dtype to
      match what's physically on disk. Measured effect on pruning:
      <https://openclimatefix.github.io/nged-substation-forecast/architecture/performance/#bounding-feature-engineering-memory-prune-the-inputs-not-the-output>.
