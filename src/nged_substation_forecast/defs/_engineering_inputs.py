...
def load_engineering_inputs(
    ...
    **Memory: prune the NWP scan at the source.** Every filter below is applied directly to the
    ``Nwp.scan_delta`` scan, so only the surviving rows are ever decoded into memory — the
    difference between a few GB and an OOM. ``init_time`` is a true partition prune;
    ``ensemble_member`` and ``h3_index`` are applied at the scan too, and ``h3_index`` narrows to
    the few cells the requested series occupy (a many-to-one relationship: one cell covers several
    series, replicated back out by the feature engineer's spatial join). See [Bounding
    feature-engineering memory: prune the inputs, not the
    output](https://openclimatefix.github.io/nged-substation-forecast/architecture/performance/#bounding-feature-engineering-memory-prune-the-inputs-not-the-output)
    for what actually prunes the scan and the measured numbers.
