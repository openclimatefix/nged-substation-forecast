...
def create_plotting_asset(model_name: str):
    """Factory to create a plotting asset for a specific model."""
    ...
    def forecast_vs_actual_plot(
        ...
        if not substation_ids:
            ...
            unique_ids = (
                forecasts.select("substation_number").unique()["substation_number"].to_list()
            )
            substation_ids = random.sample(unique_ids, min(5, len(unique_ids)))

        # Filter data
        plot_forecast = forecasts.filter(pl.col("substation_number").is_in(substation_ids))
        ...
        plot_path = settings.nged_data_path / "plots" / model_name / "forecast_vs_actual.html"
    ...
    return forecast_vs_actual_plot
