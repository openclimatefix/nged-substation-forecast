...
| `blend_products.py` | Asks whether a model shown several weather products at once beats the best single product, for solar and for wind, on the two weather-product studies' own rows, folds and seeds. Refits their single-product arms and stops unless those reproduce the published per-row losses bit for bit, then blends each set of products four ways: XGBoost on every product's columns, XGBoost on the products' mean, a cross-fitted linear stack of the single-product predictions, and their equal-weight mean. Each XGBoost blend is scored against a climatology control whose extra columns are permuted within each site, month and hour of day. Writes to `data/studies/beam_diffuse_split/blend_products/`: `losses.parquet` (every arm's per-row capped signed error and capacity-normalised absolute error, with site label, time, month, fold, seed, arm, setting and domain), `predictions.parquet` (every arm's out-of-fold prediction beside the measured power), `stack_weights.parquet` (each stack's weight on each model per site, seed and fold, cross-fitted and in-sample), `intervals.parquet` (every bootstrap interval the report prints), `reproduction.md` (the reproduction check) and `report.md`. `--resume` reuses the per-arm fits a crashed run left in `fits/`. |
