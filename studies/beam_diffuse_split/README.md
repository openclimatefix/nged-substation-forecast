...
| `weather_products.py` | Scores weather products on common row sets, one panel of products at a time, with the folds cut inside each UKV era, and answers which product best describes past sunshine. The first six products are the `published` panel. The `long` panel adds SARAH-3 and ICON-DREAM-EU and is what the sunshine write-up reports; the `record` panel holds the four products whose records reach back to 2021, for the year-by-year comparison with ERA5; the `all` panel adds the four Open-Meteo models once they are fetched. Every panel other than `published` writes under `data/studies/beam_diffuse_split/past_weather_v2/`, one directory per panel, with an `era5_by_year.parquet` table of ERA5's error against every other product by calendar year. `--concurrent-fits` shares the machine with another run. |
| `extract_site_series.py` | Cuts each solar farm's hourly global and direct irradiance from the gridded SARAH-3 and ICON-DREAM-EU downloads, at the nearest cell, and turns SARAH-3's half-hourly snapshots and ICON-DREAM's running means into means over the hour ending at each label. Writes the same per-site frame `fetch_open_meteo_point.py` writes. Takes `--product`. |
| `check_new_products.py` | Checks the per-site frames of the six products the sunshine study's second round adds: that each one's timestamps name the hour ending at the label, measured against the sun, and, for a person to read, whether each direct flux is more than a separation model, where each archive's run switches fall, and each product's monthly ratio to CAMS. Writes `product_checks.md` under `data/studies/beam_diffuse_split/past_weather_v2/`. |
| `weather_product_charts.py` | Draws the thirteen anonymised charts on the sunshine write-up from the `long` and `record` panels' reports, and bootstraps the two sets of numbers the report lacks after reproducing a printed row. Optimise each SVG with `npx svgo@4 --multipass --precision=1 --final-newline` before committing it. |
| `wind_product_charts.py` | Draws Figures 4a, 5a, 6, 7a, 8, 9, and 10 of [Which weather product best describes past wind?](https://openclimatefix.github.io/nged-substation-forecast/studies/past-weather/wind/), from `wind_products.py`'s report and its `--era5-by-year` table. |
| `past_wind_leaderboard.py` | Reads the saved losses of the four past-wind row sets (main, ICON-DREAM-EU, ECMWF, and weather station) and writes the leaderboard and contrasts report that Figures 1 and 2 of the past-wind page come from, to `data/studies/beam_diffuse_split/past_weather_v2/wind_leaderboard_2/`. It stops unless it reproduces every number that each row set's own report prints. |
| `past_wind_leaderboard_charts.py` | Draws Figures 1 and 2 of the past-wind page from that report. |
| `wind_icon_dream.py` | Extends the wind-products comparison with ICON-DREAM-EU, DWD's own reanalysis run of the ICON model, read from a gridded download (`data/studies/weather/ICON-DREAM-EU/`) rather than fetched at each generator's coordinates. Refits every one of the five original products' arms on ICON-DREAM-EU's own row set, since its record stops 10 days short of the other five, and runs the pre-fit checks issue #841 calls for: the NaN filter and duplicate-key gate, wind speed from `U`/`V` against served `WS`, direction against ERA5's and against the other three products' as a same-method baseline, and the hour-to-hour offset scan. `--report-only` rebuilds `report.md` from the saved `losses.parquet` alone, fitting nothing, but still raises if the saved fingerprint no longer matches the row set, columns, seeds and hyperparameters this code would fit; a fresh run refuses to overwrite an existing `losses.parquet` or `report.md`, so each has to move to a `superseded/` subfolder first. |
| `wind_icon_dream_charts.py` | Draws the leaderboard and planned-contrasts charts for `wind_icon_dream.py`'s report. |
...
| `fetch_ens_forecast_horizons.py` | Extracts every ECMWF ENS member's radiation, temperature, and 10 m and 100 m wind at each metered generator's H3 cell, at the leads the horizon study scores, from the production NWP Delta table, into `data/studies/ens_forecast_horizons/ens_members.parquet`. |
| `ens_forecast_horizons.py` | Scores ENS-driven power forecasts at eight horizons, for solar and wind, at hourly resolution on the past-weather studies' own rows: first how to upsample ENS's 3- and 6-hourly steps to hourly, then three ways of using the 51 members (the control member, the ensemble mean, and each member through the power model), four baselines that read no weather forecast, and two past-weather references that are not forecasts. Writes its losses, predictions, member-forecast summaries, intervals, leaderboard, and `report.md` to `data/studies/ens_forecast_horizons/`. |
| `ens_forecast_charts.py` | Draws the ENS horizon page's anonymised charts from `ens_forecast_horizons.py`'s outputs, checking each number against the report. |
| `weather_product_domains.py` | Draws the map of the ICON-D2 and AROME France domains over the UK on [Data sources](https://openclimatefix.github.io/nged-substation-forecast/roadmap/data-sources/#weather-data), from the five GeoJSON outlines in `domains/`. Its module docstring names where each outline came from. |
...
write-up](https://openclimatefix.github.io/nged-substation-forecast/studies/past-weather/solar/)
