...
| `wind_icon_dream.py` | Extends the wind-products comparison with ICON-DREAM-EU, DWD's reanalysis, read from a gridded download (`data/studies/weather/ICON-DREAM-EU/`) rather than fetched at each generator's coordinates. Refits every one of the five original products' arms on ICON-DREAM-EU's own row set, since its record stops a month short of the other five, and runs the pre-fit checks issue #841 calls for: the NaN filter and duplicate-key gate, wind speed from `U`/`V` against served `WS`, direction against ERA5's, and the hour-to-hour offset scan. `--resume` reuses `losses.parquet` if its saved fingerprint still matches; `--report-only` rebuilds `report.md` alone. |
