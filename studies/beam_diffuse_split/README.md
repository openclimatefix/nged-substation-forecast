...
| `weather_products.py` | Scores weather products on common row sets, one panel of products at a time, with the folds cut inside each UKV era, and answers which product best describes past sunshine. The first round's six products are the `published` panel; the second round's `long`, `all` and `record` panels add SARAH-3, ICON-DREAM-EU, ECMWF-IFS-HRES, ARPEGE Europe and the two HARMONIE-AROME models, and write under `data/studies/beam_diffuse_split/past_weather_v2/`, one directory per panel, each with an `era5_by_year.parquet` table of ERA5's error against every other product by calendar year. Written up as [Which weather product best describes past sunshine?](https://openclimatefix.github.io/nged-substation-forecast/studies/weather-products-for-past-solar/). |
