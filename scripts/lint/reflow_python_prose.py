...
- **Prose comment blocks** — a run of two or more consecutive whole-line `#` comments at the
  same indent, where at least one line runs past `WIDTH`. A block of that shape is wrapped with
  the same greedy word-wrap `markdown_wrap._wrap` uses. A block is refused outright wherever
  repacking it would destroy what the lines already carry: a linter directive (`noqa`, `fmt:
  off`, `type:`), a shebang, a blockquote or doctest `>`, a banner rule of dashes, a line whose
  whole content is one URL, an embedded second `#`, a run of hand-aligning spaces, or a tab. A
  single trailing inline comment needs no rule of its own, because a trailing inline comment is
  never part of a run of two whole-line comments.

Neither kind of text is touched by `ruff format`, which reformats code but leaves comments and
string literals as the author wrote them. `pyproject.toml`'s `select` list turns on ruff's `E5`
line-length rule for exactly that reason, and its comment there says so. This script exists
because nothing else in the toolchain does this reflow.

A docstring or comment block is reflowed at `WIDTH` minus its own indentation. The indent is real
characters on the source line, and counts against the 100-character budget the same as the prose
does. Reflowing operates on the dedented text, so `markdown_wrap.reflow_text`'s own units
(paragraphs, list items, fenced code) still work the way they do for a `.md` file. The result is
then re-indented, and the original closing-quote style is preserved — same line as the last word,
or its own line.
...
def _dedent_lines(lines: list[str]) -> tuple[list[str], int]:
    ...
    first line of a comment block once its `#` marker is stripped. `lines[0]` is therefore
    excluded from the margin calculation, mirroring `lint_docstring_markdown._dedent_docstring`.
    """
    margin = sys.maxsize
    for line in lines[1:]:
...
def _reflow_indented(body_lines: list[str], margin: int, *, extra_indent: int = 0) -> list[str]:
    ...
    the module-level `WIDTH` and has no width parameter of its own. The narrowing is needed
    because two further prefixes have to fit inside the same 100-character budget as the text:
    the indent this function re-adds afterwards, and any further prefix the caller re-adds
    itself. That caller prefix is `extra_indent` — a comment's `#` marker, the space after it,
    and the comment's own indent, all of which `_reflow_comment_block` adds after this function
    returns.
    """
...
def _reflow_docstring(content: str, quote_width: int) -> str:
    ...
    escaped-and-evaluated string value. This function makes a text edit to the source, not to the
    ...
    `quote_width` is `len(prefix) + len(quote)`, which is 3 for a plain triple-double-quote.
    `quote_width` is spent as `extra_indent`, to hold the opening quote's own budget for
    whichever line the first paragraph wraps onto. That line is not always line one: a docstring
    with no blank line after its summary reflows the summary into the same paragraph as the body.
    Every docstring in a `**/tests/**` file is that case, because `pydocstyle`'s `D205` — which
    requires the summary to be the entire first physical line — does not run over those files.

    A docstring with no blank line anywhere in its body reserves `quote_width` *twice* instead.
    That docstring is the same `D205`-exempt case, since a D205-compliant docstring always has a
    blank line after its summary. Its whole body is one paragraph, short enough that `ruff
    format` may later collapse the body onto a single physical line carrying both quotes,
    independently of how this function packed the text. The closing quote's width has to be held
    for that collapsed line too, or the collapsed line overflows `WIDTH`. Reserving `quote_width`
    twice on every docstring, not only on the `D205`-exempt case, would push a `D205`-compliant
    summary that already fits `WIDTH - margin - quote_width` over the narrower budget, and wrap
...
def _char_col(line: str, byte_col: int) -> int:
    ...
    `line[byte_col:]`-style slicing needs the converted character index to land on the right
    character instead. `ast.col_offset`/`end_col_offset` are documented as UTF-8 byte offsets,
    not character offsets. This repo's prose deliberately keeps en-dashes and typographic quotes,
    and `RUF001`-`RUF003` are disabled for exactly that reason. A docstring spanning a line with
    an en-dash or a typographic quote therefore needs this conversion. Without the conversion,
    the splice point lands mid-character, or lands short by however many extra bytes those
    characters cost.
    """
...
def _reflow_docstrings_in(source: str, tree: ast.Module) -> str:
    ...
    any docstring. Then splices in reverse order, highest offset first, so an earlier edit's
    offsets stay valid while a later edit is applied. Any script making several edits to one text
    works back to front for the same reason.
    """
...
def _is_prose_comment(text: str) -> bool:
    ...
    `text` still carries its own leading `#`. Every pattern matched at the start of the line
    needs that `#` anchored there. `NOT_PROSE` is checked against `text[1:]` instead, because
...
def _reflow_comment_block(lines: list[str], indent: int) -> list[str] | None:
    ...
    its text. `ruff format` already enforces that convention on every comment in this repo.
    ...
    `ruff`'s `E501` already keeps an ordinary comment inside `WIDTH`. Every comment line past
    `WIDTH` in this repo is refused on some other ground anyway: a bare URL (`URL_ONLY_COMMENT`),
    the hand-aligned sample key in `packages/nged_data/src/nged_data/storage.py` (`NOT_PROSE`),
    or a trailing inline comment, which can never be part of a run of two. So the restriction
    skips no reflow this repo wants, and leaves the script ready for the first comment that does
    overflow.
    """
    if all(len(line.rstrip("\n")) <= WIDTH for line in lines):
        return None
    # A tab is either hand alignment or an indent this function would re-emit as spaces.
    # `NOT_PROSE`'s two-space test cannot see hand alignment. So leave a block carrying a tab
    # exactly as written.
...
def _flatten(text: str) -> str:
    ...
    rewrap from a change to the words themselves. A legitimate rewrap changes how many `#`
    markers a block prints.
    """
...
def reflow_python_prose(source: str) -> str:
    ...
    and all whitespace is stripped from both. The check is the same round-trip check
