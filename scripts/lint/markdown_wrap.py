...
def _reflow_unit(unit_lines: list[str]) -> list[str]:
    ...
    opens with `> -` followed by a space, and carries `>` on every line it wraps onto. Stripping
    the marker first is what lets that item keep its own marker, rather than being read as
