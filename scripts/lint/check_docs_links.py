...
the builtins mkdocs always adds, and reads the heading ids the converter actually assigned.
...
def _strip_trailing_self_paired_run(url: str) -> str | None:
    ...
    that closes right after a URL puts three trailing double-quote characters after the URL, the
    ...
    attached. Counting the run once, up front, and removing the whole run in one step avoids that
    flip.
    """
...
def _check_link(
    ...
    in preference to a `bad_page` outcome, because a resolving page with a wrong anchor tells the
