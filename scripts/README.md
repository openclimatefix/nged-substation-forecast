...
- `check_marimo_notebooks.py` — **gate.** Checks that every name a marimo notebook's cells reference
  is bound by some cell. An unbound name is the failure `ruff check --fix` and `marimo check --fix`
  each leave behind, and the failure ruff, ty, and pytest all pass over in silence. The script runs
  as a pre-commit hook, and `tests/test_marimo_notebooks.py` runs the same check in the test suite.
