...
def test_apply_lag_feature_with_source():
    ...
    result = cast(
        pl.DataFrame,
        apply_lag_feature(target_df.lazy(), source_df.lazy(), lag_feat).collect(),
    )
...
def test_nullify_leaky_lags():
    ...
    result = cast(pl.DataFrame, nullify_leaky_lags(lf, leaky_features).collect())
...
def test_lag_feature_validation():
    ...
    lag_feat_kwargs = LagFeature(base_col="power", hours=24, string_repr="power_lag_24h")
...
def test_rolling_feature_validation():
    # Test parsing from string using from_str
    rolling_feat = RollingFeature.from_str("temperature_2m_rolling_mean_6h")
    assert rolling_feat.base_col == "temperature_2m"
    assert rolling_feat.hours == 6

    # Test parsing from keyword arguments
    rolling_feat_kwargs = RollingFeature(
        base_col="temperature_2m", hours=6, string_repr="temperature_2m_rolling_mean_6h"
    )
    assert rolling_feat_kwargs.base_col == "temperature_2m"
    ...
    with pytest.raises(ValidationError):
        RollingFeature.from_str("temperature_2m_rolling_mean_0h")

    with pytest.raises(ValidationError):
        RollingFeature.from_str("temperature_2m_rolling_mean_17521h")
...
def test_apply_rolling_mean_feature():
    ...
    result = cast(
        pl.DataFrame, apply_rolling_mean_feature(df.lazy(), "temperature_2m", 2).collect()
    )
