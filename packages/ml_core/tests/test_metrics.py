...
import polars as pl
import pytest
from contracts.ml_schemas import CvPowerForecast, Metrics
    ...
    df = pl.DataFrame(
        {
            ...
            "fold_id": pl.Series([fold_id] * n, dtype=pl.Int8),
        }
    )
    return CvPowerForecast.validate(df, allow_superfluous_columns=True)
...
def test_compute_metrics_returns_metrics_schema():
    times = [_utc(2021, 1, 1, 0, 0), _utc(2021, 1, 1, 0, 30)]
    actuals = _make_actuals(1, times, [10.0, 10.0])
    forecasts = _make_cv_forecasts(1, times, [12.0, 8.0])
    result = compute_metrics(
        CvPowerForecast.validate(forecasts, allow_superfluous_columns=True), actuals
    )
    ...
    df = pl.DataFrame(
        {
            ...
            "fold_id": pl.Series([1, 1], dtype=pl.Int8),
        }
    )
    forecasts = CvPowerForecast.validate(df, allow_superfluous_columns=True)
...
def test_compute_metrics_multiple_folds():
    ...
    times = [_utc(2021, 1, 1, 0, 0), _utc(2022, 1, 1, 0, 0)]
    actuals = _make_actuals(1, times, [10.0, 10.0])
    fold1 = _make_cv_forecasts(1, [times[0]], [15.0], fold_id=1)
    fold2 = _make_cv_forecasts(1, [times[1]], [10.0], fold_id=2)
    forecasts = CvPowerForecast.validate(pl.concat([fold1, fold2]), allow_superfluous_columns=True)
    result = compute_metrics(forecasts, actuals)
    fold1_mae = result.filter((pl.col("fold_id") == 1) & (pl.col("metric_name") == "mae"))
    fold2_mae = result.filter((pl.col("fold_id") == 2) & (pl.col("metric_name") == "mae"))
...
def test_compute_metrics_raises_on_no_join():
    ...
    actuals = _make_actuals(1, [_utc(2020, 1, 1)], [10.0])
    forecasts = CvPowerForecast.validate(
        _make_cv_forecasts(1, [_utc(2021, 1, 1)], [10.0]), allow_superfluous_columns=True
    )
