...
def list_timeseries_json_files(
    ...
    the offending file and carries on: a malformed reading originates upstream of our pipeline, at
    the meter or in the telemetry export, but a key we cannot parse means NGED's naming convention
    has changed, so every `start_time`, `end_time` and `time_series_id` this function returns is
    then suspect — including the ones parsed from the keys that still match.
    """
