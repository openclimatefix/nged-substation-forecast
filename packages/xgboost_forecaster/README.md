...
Training and prediction are **memory-bounded**: rather than collecting the whole `AllFeatures` frame, `train()` (given the `time_series_id` population) and `predict()` `collect()` **one series at a time** — filtering `data` to a single `time_series_id`, a predicate Polars pushes down into the scans, so the feature-engineering join only ever runs for one series. Each series feeds an `xgb.QuantileDMatrix`, which compresses to 8-bit quantile bins rather than holding an uncompressed `Float32` copy. Peak memory therefore stays bounded no matter how many series there are. For the full numbers and the table of which Polars operations survive the join, see [Polars push-downs through the join](../../docs/architecture/overview.md#polars-push-downs-through-the-join).
