...
@app.cell
def _():
    # Lazy: nothing is read until a cell below collects. nwp_model_id is dropped because the
    # table holds one NWP model today, so the column carries no information here.
...
@app.cell
def _(df):
    # One run at one H3 cell, as a line per ensemble member. 599148110664433663 is a resolution-5
    # cell in Shetland, at roughly 60.6 N, 0.7 W. init_time is a Delta partition column, so the
    # scan prunes to a single daily partition; h3_index sorts last, so it prunes little further.
...
def _(ENS_MEMBER_TO_PLOT, NWP_INIT_TIME, VALID_TIME_TO_PLOT, df):
    # One value per H3 cell: one run, one member, one valid time, every cell in the grid. That is
    # what the hexagon map below shades.
    filtered_df = cast(
        pl.DataFrame,
        df.filter(
            pl.col("init_time") == NWP_INIT_TIME,
...
def _(NWP_VAR_TO_PLOT, filtered_df):
    ...
    # drawn. Despite its name, max_bound holds the *range* rather than the maximum.
