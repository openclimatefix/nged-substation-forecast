...
prints each interval to a fixed number of decimal places (four for most reports, three for the
station report), which the check reads from the report's own brackets. The page quotes two.
Rounding the printed digits would turn 0.20466 into 0.2047 and then into 0.21, so the check reads
the study's `intervals.parquet` (one row per interval the report prints). Where a report number is
the print of an interval value at that precision, the check rounds that value's own digits. A
report number with no row in `intervals.parquet` is rounded from its printed digits, using
`decimal` so that binary floating-point error cannot move a half.
...
DEFAULT_INTERVAL_PRINT_DECIMALS: Final[int] = 4
"""The decimal places assumed for a report that prints no bracketed interval to read them from."""
...
def interval_print_decimals(*, report_text: str) -> frozenset[int]:
    """Return the decimal places at which a report prints its bracketed intervals.

    Args:
        report_text: The report's text.

    Returns:
        The distinct digit counts after the point among the numbers inside the report's `[a, b]`
        pairs, or `DEFAULT_INTERVAL_PRINT_DECIMALS` alone when the report has no pair.
    """
    decimals = {
        _decimals(printed=end) for pair in _report_pairs(report_text=report_text) for end in pair
    }
    return frozenset(decimals or {DEFAULT_INTERVAL_PRINT_DECIMALS})


def full_precision_values(
    *, intervals_path: Path | None, print_decimals: frozenset[int] = frozenset()
) -> ExactValues:
    """Map each number the report prints for an interval to the full-precision values behind it.
    ...
    Args:
        ...
        print_decimals: The decimal places the report prints intervals at, from
            `interval_print_decimals`. Empty means `DEFAULT_INTERVAL_PRINT_DECIMALS`.

    Returns:
        Each printed token, such as `+0.2047`, to the full-precision values that print as it. A
        token can stand for more than one value, because two intervals can agree to the printed
        precision.
    """
    ...
    for column in ("value", "lower", "upper"):
        for value in frame[column].to_list():
            for decimals in print_decimals or {DEFAULT_INTERVAL_PRINT_DECIMALS}:
                for token in (f"{value:+.{decimals}f}", f"{value:.{decimals}f}"):
...
def check_page_numbers(
    *,
    ...
    exact = full_precision_values(
        ...
        print_decimals=interval_print_decimals(report_text=report_text),
    )
