...
def test_overwrite_is_partition_scoped_by_fold_id(tmp_path: Path) -> None:
    ...
    write_eligible_time_series(_make_eligible("fold_a", [1, 2]), table, "fold_a")
    write_eligible_time_series(_make_eligible("fold_b", [3]), table, "fold_b")
    write_eligible_time_series(_make_eligible("fold_a", [4, 5, 6]), table, "fold_a")
...
def test_empty_frame_still_clears_partition(tmp_path: Path) -> None:
    ...
    write_eligible_time_series(empty, table, "fold_a")
