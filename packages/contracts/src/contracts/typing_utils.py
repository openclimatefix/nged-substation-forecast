...
def typeddict_to_dict(td: Mapping[str, object] | None) -> dict[str, str] | None:
    ...
    then call this at the last moment, at the boundary to a third-party API annotated ``dict[str,
    str]`` (or ``dict[str, Any]``). ``None`` passes straight through so a call site can forward
    an optional value unchanged.
