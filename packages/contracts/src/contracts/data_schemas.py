...
class Nwp(pt.Model):
    ...
    valid_time: datetime = pt.Field(dtype=pl.Datetime(time_zone="UTC"))
    ensemble_member: int = pt.Field(dtype=pl.Int16)
    h3_index: int = pt.Field(dtype=pl.UInt64)

    # Variables stored as int16 on disk (UInt8 is currently problematic in the Delta writer)
    ...
    downward_long_wave_radiation_flux_surface: int = pt.Field(dtype=pl.Int16)
    downward_short_wave_radiation_flux_surface: int = pt.Field(dtype=pl.Int16)
    precipitation_surface: int = pt.Field(dtype=pl.Int16)
    categorical_precipitation_type_surface: int = pt.Field(dtype=pl.Int16)
