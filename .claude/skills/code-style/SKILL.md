...
description: >-
  ...
  comment and doc-link rules, and Polars style. Load before writing or editing any Python in this
  repo — a new module, a new function, a refactor, a test — and before reviewing Python for style,
  or answering a question about what the house style is.
---
...
gets read, not to restate it, because two copies of a style guide drift apart and then nobody knows
...
about: **never row-count a table that can exceed 2³² rows with Polars** (counts wrap with no error),
and **comments must describe only how the code works now** — never a previous iteration or a deleted
file.

If you are about to write Polars or Patito code, load **`polars-patito-gotchas`** as well; the style
rules and the traps are separate documents on purpose.

## When you change the rules

Edit `docs/architecture/code-style.md` — never copy a rule into `CLAUDE.md` or into this skill. The
