...
down, and it is the whole of the draft PR's diff when step 4 opens that PR, so the reviewer reads
the plan before any code exists and later sees the plan beside the code that claims to follow it.
It is deleted at ship time into the PR body, per the existing rule.
