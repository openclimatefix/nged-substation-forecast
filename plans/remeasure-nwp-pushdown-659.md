...
**The problem: four places in the repo describe what the member-early on-disk sort buys a
...
and neither is named in the issue.** Meanwhile one location the issue does name needs nothing.
...
| `docs/architecture/performance.md:76` and `:81` | "needs re-measuring"; `:81` repeats the false mechanism | Replace both |
...
each Parquet file's `ensemble_member` row-group statistics:

| Requested member | Row groups kept | Rows decoded |
|---:|---:|---:|
| 0 (control) | 13.9% | **14.2%** |
| 1 | 23.6% | 24.4% |
| 10 | 73.6% | 75.9% |
...
**Two conclusions the docs do not currently state, and must.**

**The row groups are not member-narrow.** The median row group spans **35.5 of the 51 members**
(maximum 50), so `NWP_SORT_COLS`' claim that each row group "spans only a handful of member values
instead of all ~51" is false on the real table. `write_nwp` does sort the frame, but delta-rs then
splits ~7.24M rows across two Parquet files and redistributes the Arrow record batches
non-contiguously, so the ordering does not survive to the row-group boundaries.
`test_on_disk_format` misses this because it writes 6 rows into a single file.

**The pruning that does happen is an artifact of the control member sitting at the end of the
range.** A row group spanning members 13 to 48 excludes 0, so it is skipped — not because it is
narrow, but because 0 is the minimum. That is why members 0 and 50 prune well and member 25 prunes
essentially not at all. **Any future per-member training run that asks for a mid-range member gets
no row-group pruning**, which is a real limitation and the single most useful thing this measurement
produces.
...
1. Read one `init_time` partition back through `Nwp.scan_delta`, and write it twice under
   `~/.cache/nged-nwp-sort-benchmark/` — once through the real `write_nwp`, once with
   `NWP_SORT_COLS` monkeypatched to the previous ordering, `("init_time", "valid_time",
   "ensemble_member", "h3_index")`. Git history confirms that is the ordering the member-early sort
   replaced (`dynamical_data`, before commit `83a40738`).
2. Compare the two arms on row groups surviving `member == 0`, rows decoded, and Parquet bytes.
   **Both arms must be freshly written**; reading the member-early arm from the production table
   would confound sort order with that table's file and row-group boundaries.
3. Census the member-survival table above over ~10 real partitions, read-only from row-group
   statistics. One partition is enough for the two-arm write, but the survival percentage varies
   partition to partition and ~10 costs seconds.
4. Time the production read shape — `Nwp.scan_delta(table).filter(init_time in window,
   ensemble_member == 0, h3_index.is_in(cells)).collect(engine="streaming")` — on each arm, as a
   warm-cache median after a warm-up. The 9 cells come from the `h3_res_5` column of
   `data/NGED/metadata.parquet`, which holds 33 series across 9 unique cells.
5. Measure peak resident memory once per arm, each in **its own subprocess**:
   `resource.getrusage(...).ru_maxrss` is a high-water mark that never falls within a process, so
   both arms in one process would report the larger figure twice. `tracemalloc` is not an
   alternative — Polars allocates outside Python's allocator.
6. Delete the scratch tables. Peak scratch usage is about 300 MB.
...
the same way as the docstring, and its "peak-memory reduction versus a `valid_time`-first sort"
clause is deleted — it contradicts the same paragraph's own point that the streaming engine bounds
peak memory by morsel size rather than window length.
...
a property the real write path does not have and gone green. The mechanism keeps the coverage it
already has: `test_on_disk_format` asserts the sort is applied, and
`test_scan_pushes_filters_into_the_parquet_scan` asserts the predicate reaches the scan.
...
**Should the layout be fixed so the sort actually survives to the row groups?** `write_nwp` sorts
correctly, but delta-rs scatters the batches, so the intended mechanism is not what delivers the
pruning. Fixing it (a `max_rows_per_file` or single-writer setting in `delta_store.nwp`) would
change the very numbers #659 writes down. **Recommendation: out of scope here — open it as its own
issue and sequence it after #659**, so this issue records what the table does today and that issue
changes it. Worth the maintainer's call, since it is the most valuable thing the measurement found.

**Should the member-survival table go in the README, or is it more than the docs need?** It is the
only evidence that the benefit does not generalise beyond the control member. **Recommendation:
include it** — a future per-member training run would otherwise assume pruning it will not get.

**What if the wall-time difference is not measurable?** The read returns about 21,000 rows, so fixed
costs may dominate and the two arms may time the same despite the decode difference.
**Recommendation: say exactly that, and lead with rows decoded rather than a wall-time ratio.**
Reporting "no measurable difference at this scale" is a finding, not a failed measurement.
