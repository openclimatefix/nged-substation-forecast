...
- `store.py` — crop to the box and append each run to the product's Icechunk repository, one repository per product, on the Source Cooperative bucket directly. Each variable is one array with dimensions `(init_time, member, step, cell)` (ICON-D2 and ICON-ART-EU have no `member`), so a study opens one array per product and slices it. Variables whose step list differs (ICON-D2 radiation every 15 minutes, ICON-ART-EU `TAOD_DUST`) sit in their own group with their own `step` coordinate. The grid coordinates `clat`, `clon`, `hsurf`, `fr_land` and `hhl` (levels 62-64 for ICON-D2 and ICON-D2-EPS, 72-74 for ICON-EU-EPS) are stored once per product, and the recorder compares each run's grid with the stored one and reports a mismatch through Sentry. The raw GRIB files of a run are cached in a local directory as they arrive (this is the checkpoint), and once the run is complete or its deadline has passed the recorder builds the run member by member, so each per-member chunk `(1, 1, 1, steps, cells)` is written once and never rewritten. Values get 13-bit significand rounding in numpy before the write, and the codecs are `zstd` with the Icechunk defaults. One Icechunk commit per run is the commit: its message and the `init_time` coordinate carry the status (`complete` or `partial`), expected and received file counts, code version, ICON model version and archive time, and a run that is absent past its deadline is a NaN slice plus a Sentry event. After the commit succeeds, the local raw files are deleted and a small tombstone is kept so the run is not fetched again. There is no garbage collection and no snapshot expiry, because the archive never deletes.
...
- `test_store.py`: a synthetic triangular grid keeps exactly the cells in the box, and a cell 0.01 degrees outside is dropped; writing the same run twice gives identical decoded arrays; a bitmapped message decodes to NaN, never 9999; against moto (reset per test) an Icechunk commit interrupted before it completes leaves the previous snapshot readable and the run still expected, and appending the same run twice leaves one `init_time` slice with identical values; a run built member by member equals the same run written all at once.
