...
- **Start recording before the account exists?** The DWD runs vanish in 24 hours. Recommendation: yes, but upload to a bucket in our own AWS account in `eu-west-2` from day one and copy to Source Cooperative later, because the DWD products alone come to 14-23 GB a day stored, so 100 GB of local disk lasts only 4-7 days and a full disk is the failure the recorder cannot degrade around. S3 Standard in London is about 0.024 USD per GB-month, so the first month is a few pounds. The maintainer's AWS account is needed for this bucket.
