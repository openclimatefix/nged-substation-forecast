...
- `crop.py` — the "fat Great Britain" box (49.0-61.5 N, 10.0 W-3.5 E), chosen by the maintainer to reach offshore wind farms as well as land. It covers Northern Ireland, the Irish Sea, the Celtic Sea off Cornwall, the seas west of the Hebrides, Shetland, and the North Sea out to the Dutch and Belgian coasts (Dogger Bank at about 2 E and the Hornsea and Norfolk zones are well inside). ICON-D2-EPS only covers the part east of about 3.94 W, so its offshore coverage is the North Sea only. DWD: keep native cells whose `clat`/`clon` fall in the box and store those two coordinate arrays. MOGREPS-UK: an index box on its own projection, stored with the projection attributes.
...
The maintainer chose full horizons (ICON-EU-EPS 0-120 h, ICON-D2-EPS 0-48 h, MOGREPS-UK 0-126 h) and every member. Stored volume, for the earlier 49.8-61.0 N, 8.3 W-2.0 E box, was estimated at about 7.5 TB; the fat box is about 1.5 times that area, so roughly 11 TB (an estimate, to be replaced by the measured size of the first dry run) a year, most of it MOGREPS-UK; ICON-D2-EPS is the product to reduce (mean, spread and a few quantiles) if storage cost later matters.
...
- **Post-beta storage cost.** At S3 London list price, 11 TB is about 200 GBP a month. Storage is excluded from the 25 GBP budget by the maintainer's decision.
