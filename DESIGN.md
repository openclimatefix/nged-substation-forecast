...
- [ ]  Teach Dagster how to:
    - [x] download Live NGED substation power data from NGED's CKAN API, and locations of substations.
...
The dream: A web app (implemented in Marimo & Altair) with a small map of NGED substations (and of
any generation data NGED provide). The user can drag the map around. Click on a substation in the
map to show 7 days of time series data in a line plot on the right. The map should take up about 1/4
of the width of the screen, and the time series should take up 3/4 of the width. Show MW (or MVA if
MW isn't available) from the substation.
- [ ] Teach Dagster:
    - [ ] CERRA (or maybe save this until later)
    - [ ] ECMWF ens from Dynamical.org
...
- [ ] Build a simple XGBoost model to predict net demand per primary
    - [ ] Use MLFlow with Dagster.
- [ ] Add our demand predictions to the app.
- [ ] Add a `Sensor` to Dagster to detect new data, and trigger the pipeline.
